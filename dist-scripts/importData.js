...
const importData = async () => {
            ...
            for (const lineup of lineups) {
                ...
                if (!teamsToProcess.has(teamId)) {
                    teamsToProcess.set(teamId, {
                        _id: new mongoose_1.default.Types.ObjectId(), // Generate new ObjectId
    ...
    for (const teamData of Array.from(teamsToProcess.values())) {
        try {
            ...
            let existingTeam = await Team_1.default.findOne({ name: teamData.name });
            if (existingTeam) {
                ...
                // Create new team
                const newTeam = new Team_1.default(teamData);
                await newTeam.save();
    ...
    for (const playerData of Array.from(playersToProcess.values())) {
        try {
            ...
            let existingPlayer = await Player_1.default.findOne({ name: playerData.name, team: playerData.team });
            if (existingPlayer) {
                ...
                // Create new player
                const newPlayer = new Player_1.default(playerData);
